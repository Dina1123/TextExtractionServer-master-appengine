...
function filterRequest($requestname)
{
    if (isset($_POST[$requestname])) {
        return htmlspecialchars(strip_tags($_POST[$requestname])); // sanitize and return the requested value
    } else {
        // Handle cases where the requested parameter is not provided
        echo json_encode(["status" => "error", "message" => "$requestname not provided"]);
        exit;
    }
}
...
function imageUploadToGCS($imageRequest)
{
    ...
    if (empty($msgError)) {
        // Google Cloud Storage upload logic
        $storage = new StorageClient();
        $bucketName = 'text-402508.appspot.com'; // Replace with your actual GCS bucket name
        $bucket = $storage->bucket($bucketName);
